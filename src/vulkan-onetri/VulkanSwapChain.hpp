...
public:
	...
	VkSwapchainKHR                  swapChain = VK_NULL_HANDLE; /** @brief Handle to the current swap chain, required for recreation */
	uint32_t                        imageCount;
	std::vector<VkImage>            images;
	std::vector<SwapChainBuffer>    buffers;
	uint32_t                        queueNodeIndex = UINT32_MAX; /** @brief Queue family index of the detected graphics and presenting device queue */
	...
	void create(uint32_t *width, uint32_t *height, bool vsync = false) {
		...
		swapchainCI.imageExtent = VkStruct::extent2D ( swapchainExtent.width, swapchainExtent.height );
		...
		for (uint32_t i = 0; i < imageCount; i++) {
			...
			colorAttachmentView.components = VkStruct::componentMapping (
