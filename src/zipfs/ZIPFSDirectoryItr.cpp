...
//----------------------------------------------------------------//
char const* ZIPFSDirectoryItr::GetEntryName () {

	return this->mName;
}

//----------------------------------------------------------------//
bool ZIPFSDirectoryItr::IsSubdir () {

	return this->mIsDir;
}

//----------------------------------------------------------------//
int ZIPFSDirectoryItr::Open () {

	ZIPFSFileSystem& filesys = ZIPFSFileSystem::Get ();

	this->mDirName = filesys.GetWorkingPath ();
	ZIPFSVirtualPath* mount = filesys.FindBestVirtualPath ( this->mDirName.c_str ());
...
//----------------------------------------------------------------//
int ZIPFSDirectoryItr::ReadEntry () {
	...
	if ( this->mVirtualSubDir ) {
		...
		this->mIsDir = true;
		this->mVirtualSubDir = ZIPFSFileSystem::Get ().FindNextVirtualSubdir ( this->mDirName.c_str (), this->mVirtualSubDir );
...
//----------------------------------------------------------------//
int ZIPFSDirectoryItr::ReadZipEntry () {
...
//----------------------------------------------------------------//
ZIPFSDirectoryItr::ZIPFSDirectoryItr () :
	mZipFileDir ( 0 ),
	mZipFileSubDir ( 0 ),
	mZipFileEntry ( 0 ),
	mVirtualSubDir ( 0 ),
