...
ZLSizeResult ZLMemStream::ReadBytes ( void* buffer, size_t size ) {
	...
	if ( this->mGuestBuffer ) {
		...
		ZLIndex chunk0 = ( ZLIndex )( cursor0 / this->mChunkSize );
		ZLIndex chunk1 = ( ZLIndex )( cursor1 / this->mChunkSize );
...
ZLResultCode ZLMemStream::Reserve ( size_t length ) {
	...
	for ( ZLIndex i = totalChunks; i < neededChunks; ++i ) {
	...
	for ( ZLIndex i = 0; i < totalChunks; ++i ) {
		temp [ i ].Take ( this->mChunks [ i ]);
	}
