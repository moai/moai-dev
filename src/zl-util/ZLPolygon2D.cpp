...
size_t ZLPolygon2D::Clip ( const ZLPlane2D& plane, ZLStream& clippedPolyVerts, ZLStream& clippedPolySizes ) {
	...
	for ( size_t j = 0; j < polySize; ++j ) {
		...
		float dist = ZLDist::PointToPlane2D ( vert, plane );
	...
	struct ClipVert* clipVerts = ( struct ClipVert*  )alloca ( maxVerts * sizeof ( struct ClipVert ));
	ZLRadixKey32 < struct ClipVert* > * sortBuffer = ( ZLRadixKey32 < struct ClipVert* >*  )alloca ( maxIntersections * sizeof ( ZLRadixKey32 < struct ClipVert* > ));
	ZLRadixKey32 < struct ClipVert* > * swapBuffer = ( ZLRadixKey32 < struct ClipVert* >*  )alloca ( maxIntersections * sizeof ( ZLRadixKey32 < struct ClipVert* > ));
	
	size_t nClipVerts = 0;
	size_t nIntersections = 0;

	ZLVec2D tangent = plane.mNorm;
	tangent.Rotate90Anticlockwise ();

	bool inFront = false; // we're guaranteed to start behind or on plane thanks to startIdx

	struct ClipVert* loopStart = 0;
	struct ClipVert* prevVert = 0;

	for ( size_t j = 0; j < ( polySize + 1 ); ++j ) {
		
		size_t k = ( startIdx + j ) % polySize;
		const ZLVec2D& v1 = this->mVertices [ k ];
		
		float dist = ZLDist::PointToPlane2D ( v1, plane );
		
		//bool nextInFront = inFront ? dist >= 0.0f : dist > 0.0f;
		bool nextInFront = dist > 0.0f;
		
		if ( inFront && ( dist == 0.0f )) {
		
			const ZLVec2D& v2 = this->mVertices [( k + 1 ) % polySize ];
			if ( ZLDist::PointToPlane2D ( v2, plane ) >= 0.0f ) {
				nextInFront = true;
			}
		}
		
		if ( inFront != nextInFront ) {
		
			// v0, v1 must be different points *by definition* - one is one or behind and one is in front
			const ZLVec2D& v0 = this->mVertices [( k == 0 ? polySize : k ) - 1 ];
			
			// get the edge, normalize and check intersection with plane
			ZLVec2D e0 = v1;
			e0.Sub ( v0 );
			e0.Norm ();
			
			float t;
			u32 setResult = ZLSect::VecToPlane ( v0, e0, plane, t );
			...
			if ( setResult == ZLSect::SECT_HIT ) {
				...
				clipVert.mIntersection		= true;
				clipVert.mTangent			= tangent.Dot ( clipVert.mPoint );
				clipVert.mNextVert			= 0;
				clipVert.mNextIntersection	= 0;
				
				ZLRadixKey32 < struct ClipVert* >& sortKey = sortBuffer [ nIntersections++ ];
		...
		if ( nextInFront ) {
			...
			struct ClipVert& clipVert = clipVerts [ nClipVerts++ ];
			clipVert.mPoint				= v1;
			clipVert.mIntersection		= false;
			clipVert.mTangent			= 0.0f;
			clipVert.mNextVert			= 0;
			clipVert.mNextIntersection	= 0;
	...
	for ( size_t i = 0; i < nIntersections; ++i ) {
		...
		if ( intersection->mNextVert ) {
			...
			bool isEnd = !intersection->mNextVert;
			bool isEven = ( i & 1 ) == 0;
	
			if ( isEven ? !isEnd : isEnd ) {
				
				ClipVert* newHead = isEnd ? intersection : intersection->mOtherEnd;
				ClipVert* newTail = newHead->mOtherEnd;
				
				ClipVert* prev = 0;
				ClipVert* cursor = newTail;
				while ( cursor ) {
					ClipVert* next = cursor->mNextVert;
					cursor->mNextVert = prev;
					prev = cursor;
					cursor = next;
				}
			}
	...
	for ( size_t i = 1; i < nIntersections; ++i ) {
	...
	struct ClipVert* cursor = sortBuffer [ 0 ].mData;
	struct ClipVert* firstIntersection = cursor;
	struct ClipVert* nextIntersection = 0;
	...
	while ( cursor ) {
		...
		if ( newIntersection ) {
			...
			cursor = cursor->mNextIntersection;
			nextIntersection = cursor->mNextIntersection;
			newIntersection = false;
		}
		else {
			
			cursor = cursor->mNextVert;
			
			if ( cursor == firstIntersection ) {
				clippedPolySizes.Write < size_t >( clippedPolySize );
				clippedPolySize = 0;
				cursor = nextIntersection;
				firstIntersection = cursor;
				newIntersection = true;
			}
			
			if ( cursor && cursor->mIntersection ) {
				newIntersection = true;
				nextIntersection = 0;
			}
		}
