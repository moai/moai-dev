...
#ifndef	ZLVERTEXFORMAT_H
#define	ZLVERTEXFORMAT_H
...
protected:
	...
	ZLLeanArray < ZLVertexAttribute >		mAttributes; // this is the linear array of attributes
...
public:
	...
	//----------------------------------------------------------------//
	static ZLVertexFormat*			AffirmVertexFormat				( MOAILuaState& state, int idx );
	void							Clear							();
	...
	const ZLVertexAttribute&		GetAttribute					( ZLIndex attrIdx );
	const ZLVertexAttribute*		GetAttributeByUse				( u32 useID, ZLIndex attrIndex ) const;
	
	static size_t					PackAttribute					( void* buffer, const ZLVec4D& coord, const ZLVertexAttribute& attribute );
	...
	static ZLVec4D					UnpackAttribute					( const void* buffer, const ZLVertexAttribute& attribute, float yFallback, float zFallback, float wFallback );
	static ZLVec4D					UnpackCoord						( const void* buffer, const ZLVertexAttribute& attribute );
	...
	//----------------------------------------------------------------//
	inline void* GetAttributeAddress ( const ZLVertexAttribute& attribute, void* vertexBuffer, u32 vtxIdx ) const {

		return ( void* )this->GetAttributeAddress ( attribute, ( const void* )vertexBuffer, vtxIdx );
	}

	//----------------------------------------------------------------//
	inline const void* GetAttributeAddress ( const ZLVertexAttribute& attribute, const void* vertexBuffer , u32 vtxIdx ) const {
