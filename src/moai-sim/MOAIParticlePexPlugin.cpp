...
static void read_n_values ( float *array, TiXmlAttribute *attribute, int max, int *registers, int &size ) {
...
void MOAIParticlePexPlugin::OnRender ( float* particle, float* registers, AKUParticleSprite* sprite, float t0, float t1,  float term) {
	...
	if ( mEmitterType == EMITTER_GRAVITY ) {
		...
		if ( mRadialAcceleration != 0 || mRadialAccelVariance != 0 || mTanAccel != 0 || mTanAccelVariance != 0 ) {
...
void MOAIParticlePexPlugin::Parse ( cc8* filename, MOAIParticlePexPlugin& plugin, TiXmlNode* node ) {
	...
	if ( element && !strcmp ( element->Value (), "particleEmitterConfig" )) {
		...
		for ( ; childElement; childElement = childElement->NextSiblingElement ()) {
			...
			if ( !attribute || !text || text == "" ) continue;
...
//				STLString data = childElement->Attribute ( "data" );
//				if ( data.size ()) {
//					printf ( "floooooob! %s\n", data.c_str ());
//					size_t len = data.base_64_decode_len ();
//					if ( len ) {
//						
//						void* buffer = malloc ( len );
//						data.base_64_decode ( buffer, len );
//						
...
//						
//						inflater.SetWindowBits ( 16 + MAX_WBITS );
//						inflater.Open ( &cryptStream );
//						plainStream.WriteStream ( inflater );
//						inflater.Close ();
//						
...
//						image->Load ( stream );
//						plugin.mImage.Set ( plugin, image );
//						
//						free ( png );
//						free ( buffer );
//					}
//				}
		...
		minLifespan = minLifespan < 0.0f ? 0.0f : minLifespan;
		
		float avgLifespan = ( maxLifespan + minLifespan ) / 2.0f;
		
		plugin.mEmissionRate =  1.0f / ( plugin.mNumParticles / avgLifespan );
