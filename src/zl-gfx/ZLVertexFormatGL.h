...
#ifndef	ZLVERTEXFORMATGL_H
#define	ZLVERTEXFORMATGL_H
...
protected:
	...
	ZLLeanArray < ZLVertexAttributeGL >		mAttributes; // this is the linear array of attributes
...
public:
	...
	const ZLVertexAttributeGL&		GetAttribute					( ZLIndex attrIdx );
	const ZLVertexAttributeGL*		GetAttributeByUse				( u32 useID, ZLIndex attrIndex ) const;
	
	static size_t					PackAttribute					( void* buffer, const ZLVec4D& coord, const ZLVertexAttributeGL& attribute );
	...
	static ZLVec4D					UnpackAttribute					( const void* buffer, const ZLVertexAttributeGL& attribute, float yFallback, float zFallback, float wFallback );
	static ZLVec4D					UnpackCoord						( const void* buffer, const ZLVertexAttributeGL& attribute );
	...
	//----------------------------------------------------------------//
	inline void* GetAttributeAddress ( const ZLVertexAttributeGL& attribute, void* vertexBuffer, u32 vtxIdx ) const {

		return ( void* )this->GetAttributeAddress ( attribute, ( const void* )vertexBuffer, vtxIdx );
	}

	//----------------------------------------------------------------//
	inline const void* GetAttributeAddress ( const ZLVertexAttributeGL& attribute, const void* vertexBuffer , u32 vtxIdx ) const {
