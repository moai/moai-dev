...
jobject JniUtils::CreateObjectOfClass () {
     ...
     if ( !this->mClass ) {
		ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: Missing class; cannot create object" );
		assert ( false );
		return NULL;
    }
...
jclass JniUtils::GetClass ( cc8* className ) {
	...
	if ( clazz == NULL ) {
		ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: Unable to find java class %s", className );
		this->ClearException ();
	}
	return clazz;
}
...
jclass JniUtils::GetClassViaLoader ( cc8* className ) {
    ...
    if ( clazz == NULL ) {
		ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: Missing class; cannot find java class %d", className );
		this->ClearException ();
    }
	return clazz;
	*/
...
jmethodID JniUtils::GetMethod ( jclass clazz, cc8* methodName, cc8* methodSignature ) {
    ...
    if ( !clazz ) {
		ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: Missing class; cannot find java method %d", methodName );
		this->ClearException ();
		return NULL;
    }
	...
	if ( method == NULL ) {
		ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: Unable to find java method %s", methodName );
		this->ClearException ();
	}
	return method;
}
...
jmethodID JniUtils::GetStaticMethod ( jclass clazz, cc8* methodName, cc8* methodSignature ) {
    ...
    if ( !clazz ) {
		ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: Missing class; cannot find static java method %d", methodName );
		this->ClearException ();
		return NULL;
    }
	...
	if ( method == NULL ) {
		ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: Unable to find static java method %s", methodName );
		this->ClearException ();
	}
	return method;
}
...
bool JniUtils::SetClass ( cc8* className ) {
	...
	ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: set class %s", className );
...
bool JniUtils::SetClassViaLoader ( cc8* className ) {
	...
	ZLLog::LogF ( 1, ZLLog::CONSOLE, "MOAI JNI: set class via loader %s", className );
