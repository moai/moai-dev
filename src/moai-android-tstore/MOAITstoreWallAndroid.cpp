...
int MOAITstoreWallAndroid::_showOfferWall ( lua_State* L ) {
    ...
    if ( tstore == NULL ) {
		...
		ZLLog::Get ().LogF ( 1, ZLLog::CONSOLE, "MOAITstoreWallAndroid: Unable to find java class %s", "com/ziplinegames/moai/MoaiTstoreWall" );
    } else {

    	jmethodID showOfferWall = env->GetStaticMethodID ( tstore, "showOfferWall", "()V" );
    	if ( showOfferWall == NULL ) {

			ZLLog::Get ().LogF ( 1, ZLLog::CONSOLE, "MOAITstoreWallAndroid: Unable to find static java method %s", "showOfferWall" );
    	} else {
