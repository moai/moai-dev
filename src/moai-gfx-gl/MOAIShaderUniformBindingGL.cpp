...
//----------------------------------------------------------------//
void MOAIShaderUniformBindingGL::Bind ( ZLGfx&gfx, const MOAIUniformDescriptor& descriptor, const void* buffer ) const {

	if ( this->mGPUBase == ZGL_INVALID_UNIFORM_ADDR ) return;

	if ( descriptor.mType == MOAIUniformDescriptor::UNIFORM_TYPE_FLOAT ) {
		gfx.UniformFloat ( this->mGPUBase, 0, descriptor.mWidth, descriptor.mCount, ( float* )buffer );
	}
	else if ( descriptor.mType == MOAIUniformDescriptor::UNIFORM_TYPE_INT ) {
