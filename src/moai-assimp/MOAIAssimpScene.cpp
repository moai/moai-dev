...
int MOAIAssimpScene::_getAnimations ( lua_State* L ) {
	...
	if ( state.IsType ( 2, LUA_TNUMBER )) {
		...
		lua_newtable ( state );
		for ( ZLIndex i = 0; i < self->mScene->mNumAnimations; ++i ) {
...
int MOAIAssimpScene::_getCameras ( lua_State* L ) {
	...
	if ( state.IsType ( 2, LUA_TNUMBER )) {
		...
		lua_newtable ( state );
		for ( ZLIndex i = 0; i < self->mScene->mNumCameras; ++i ) {
...
int MOAIAssimpScene::_getMaterials ( lua_State* L ) {
	...
	if ( state.IsType ( 2, LUA_TNUMBER )) {
		...
		lua_newtable ( state );
		for ( ZLIndex i = 0; i < self->mScene->mNumMaterials; ++i ) {
...
int MOAIAssimpScene::_getMeshes ( lua_State* L ) {
	...
	if ( state.IsType ( 2, LUA_TNUMBER )) {
		...
		lua_newtable ( state );
		for ( ZLIndex i = 0; i < self->mScene->mNumMeshes; ++i ) {
...
void MOAIAssimpScene::Clear () {
	...
	for ( ZLIndex i = 0; i < this->mCameras.Size (); ++i ) {
		this->LuaRelease ( this->mCameras [ i ]);
	}
	this->mCameras.Clear ();

	for ( ZLIndex i = 0; i < this->mMaterials.Size (); ++i ) {
		this->mMaterials [ i ].Clear ();
	}
	this->mMaterials.Clear ();

	for ( ZLIndex i = 0; i < this->mMeshes.Size (); ++i ) {
...
MOAIAssimpScene::MOAIAssimpScene () :
	...
	RTTI_BEGIN ( MOAIAssimpScene )
		RTTI_VISITOR ( MOAIAbstractLuaRegistrationVisitor, MOAILuaRegistrationVisitor < MOAIAssimpScene >)
...
//----------------------------------------------------------------//
void MOAIAssimpScene::_RegisterLuaClass ( RTTIVisitorHistory& history, MOAILuaState& state ) {
...
//----------------------------------------------------------------//
void MOAIAssimpScene::_RegisterLuaFuncs ( RTTIVisitorHistory& history, MOAILuaState& state ) {
	if ( history.DidVisit ( *this )) return;
