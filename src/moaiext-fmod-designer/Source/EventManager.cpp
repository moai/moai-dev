...
{
    return (u32)(USDeviceTime::GetTimeInSeconds() * 1000.0);
}
...
void EventManager::SetMicrophoneTransform(const USVec3D& vPos, const USVec3D& vVelocity, const USVec3D& vLook, const USVec3D& vUp, const USVec3D& vRight)
{    
...
EventHandle EventManager::PlayEvent3D(const Event& soundEvent, const USVec3D& vPos, bool loopSound, const USVec3D& vVelocity, const LineCode* pLineCode)
{   
    if( soundEvent.IsValid() )
    {   
        PROFILE_TIME_REGION(sg_SoundZone);
            ...
            else
			{
				...
				{                                   
					...
					else if(pInstance->GetFlags(EventInstance::kSIF_HasCutoffRadius | EventInstance::kSIF_HasRetriggerRadius))
					{
						...
						USVec3D vSourcePosition = _GetWorldspacePosition(pInstance->m_vPosition, pInstance->GetFlag(EventInstance::kSIF_HeadRelative));
...
USVec3D EventManager::_GetWorldspacePosition( const USVec3D& vPosition, bool headRelative ) const
{
    if( headRelative )
    {
        USMatrix4x4 mtxlistener;
        USVec3D vTransformedVec;
...
void FMODDesigner::CopyDFVectorToFMOD( FMOD_VECTOR& vFMOD, const USVec3D& vDF )
{
    vFMOD.x = vDF.mX;
    vFMOD.y = vDF.mY;
    vFMOD.z = vDF.mZ;
...
void FMODDesigner::CopyFMODVectorToDF( USVec3D& vDF, const FMOD_VECTOR& vFMOD )
{
    vDF.mX = vFMOD.x;
    vDF.mY = vFMOD.y;
    vDF.mZ = vFMOD.z;
...
{
    outPath.clear();
    outPath.write( USFileSys::GetCurrentPath().c_str() );

    if ( getFullPath )
    {
        STLString mediaPath = USFileSys::GetAbsoluteDirPath( outPath.c_str() );        
        outPath = mediaPath;
...
EventManager::VoiceDataExists(const char* szProject)
{
    ...
    return (USFileSys::CheckFileExists(fsb.c_str()));
}
...
bool EventManager::_PassesRetriggerTest( USVec3D vSourcePosition, const EventProperties& properties, const Event& soundEvent, EventInstance* pInstance )
{
...
EventInstance* EventManager::_PlayEvent( const Event& soundEvent, bool loops, const USVec3D* pvPosition, EventInstance* pInstanceOverride, const LineCode* pLineCode )
{
                ...
                
        {
            ...
            if( pProperties )
            {
                ...
                else if( pProperties->m_is3D && pvPosition )
                {
                    // The source might be head relative or world space relative,
                    // so we need to be sure that we get the proper world space transform.
                    USVec3D vSourcePosition = _GetWorldspacePosition(*pvPosition, pProperties->m_headRelative);
...
void EventManager::_SetMicrophoneTransform(const USVec3D &vPos, const USVec3D& vVelocity, const USVec3D &vLook, const USVec3D &vUp )
{
#ifndef MOAI_OS_WINDOWS
    #ifndef _isnan
        #define _isnan isnan
            ...
            for( u32 i = 0; i < m_initParams.m_nVirtualChannels; ++i )
            {
                ...
                if( pSystem->getChannel( i, &pChannel ) == FMOD_OK )
                {
                        ...
                        (mode & FMOD_3D) && (mode & FMOD_3D_HEADRELATIVE) == 0 )
                    {
                        ...
                        {
                            float fDistance = m_vListenerPosition.Dist( USVec3D( position.x, position.y, position.z ) );
