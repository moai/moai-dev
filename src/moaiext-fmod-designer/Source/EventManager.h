...
namespace FMODDesigner
{
    ...
    void CopyDFVectorToFMOD( FMOD_VECTOR& vFMOD, const USVec3D& vDF );
    void CopyFMODVectorToDF( USVec3D& vDF, const FMOD_VECTOR& vFMOD );
    ...
    */
    ...
    public:
        ...
        * wavName should be only used for programmer sounds (a very advanced feature).
        */
        EventHandle PlayEvent3D(const Event& soundEvent, const USVec3D& vPos, bool loopSound = false, const USVec3D& vVelocity = USVec3D( 0.f, 0.f, 0.f ), const LineCode* pLineCode = NULL);
        ...
        */
        void SetMicrophoneTransform(const USVec3D &vPos, const USVec3D& vVelocity = USVec3D(0.f,0.f,0.f), const USVec3D &vLook = USVec3D(0.f, 0.f, 1.f), const USVec3D &vUp = USVec3D(0.f, 1.f, 0.f), const USVec3D& vRight = USVec3D(-1.f,0.f,0.f) );    

        /*! Returns the position of the microphone */
        const USVec3D& GetMicrophonePosition() const   { return m_vListenerPosition; }
        const USVec3D& GetMicrophoneVelocity() const   { return m_vListenerVelocity; }
        const USVec3D& GetMicrophoneForward() const    { return m_vListenerForward; }
        const USVec3D& GetMicrophoneUp() const         { return m_vListenerUp; }
     ...
     private:
        ...
        USVec3D _GetWorldspacePosition( const USVec3D& vPosition, bool headRelative ) const;    
        ...
        bool _PassesRetriggerTest( USVec3D vSourcePosition, const EventProperties& properties, const Event& soundEvent, EventInstance* pInstance );    
        
        EventInstance* _PlayEvent(const Event& soundEvent, bool loops, const USVec3D* pvPosition, EventInstance* pInstanceOverride, const LineCode* pLineCode);    
        ...
        void _SetMicrophoneTransform(const USVec3D &vPos, const USVec3D& vVelocity, const USVec3D &vLook, const USVec3D &vUp );
    ...
    private:    
        ...
        // Listener
        USVec3D            m_vListenerPosition;
        USVec3D            m_vListenerVelocity;
        USVec3D            m_vListenerForward;
        USVec3D            m_vListenerUp;
        USVec3D            m_vListenerRight;
