...
public:
	...
	void		LogF				( u32 level, ZLFILE* file, cc8* format, ... );
	void		LogV				( u32 level, ZLFILE* file, cc8* format, va_list args );

	void		SetLogFunc			( ZLLogFunc logFunc, void* userdata );
	void		SetLogLevel			( u32 level );
	void		SetRedirect			( ZLFILE* file );
				ZLLog				();
				~ZLLog				();
};
