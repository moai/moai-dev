...
ZLCgtSymbol* ZLCgt::FindNonterminal ( cc8* symbolName ) {
	...
	for ( ZLIndex i = 0; i < this->mSymbolTable.Size (); ++i ) {
	...
	while ( stream.GetCursor () < length ) {
		...
		switch ( contentType ) {
			...
			case 'S': {
				...
				ZLIndex index = this->ReadIntEntry ( stream );
			...
			case 'D': {
				...
				for ( ZLIndex i = 0; i < totalEdges; ++i ) {
			...
			case 'L': {
				...
				for ( ZLIndex i = 0; i < totalActions; ++i ) {
