...
//----------------------------------------------------------------//
void ZLLog::LogF ( u32 level, ZLFILE* file, cc8* format, ... ) {
...
//----------------------------------------------------------------//
void ZLLog::LogV ( u32 level, ZLFILE* file, cc8* format, va_list args ) {
	...
	if ( this->mLogLevel <= level ) {
		...
		if ( this->mLogFunc ) {
			...
			if ( file ) {
				zl_vfprintf (( ZLFILE* )file, format, args );
			}
			else {
...
//----------------------------------------------------------------//
void ZLLog::SetRedirect ( ZLFILE* file ) {
