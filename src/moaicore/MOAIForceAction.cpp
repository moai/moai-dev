...
\n
	...
	@param total (in)
*/
int MOAIForceAction::_reserveForces ( lua_State* L ) {
	LUA_SETUP ( MOAIForceAction, "UN" );
...
\n
	...
	@param delay (in)
*/
int MOAIForceAction::_setDelay ( lua_State* L ) {
	LUA_SETUP ( MOAIForceAction, "UN" );
...
\n
	...
	@param mode (in)
*/
int MOAIForceAction::_setForce ( lua_State* L ) {
	LUA_SETUP ( MOAIForceAction, "UNUNN" );
...
*/
int MOAIForceAction::_setMode ( lua_State* L ) {
	LUA_SETUP ( MOAIForceAction, "UN" );
...
//----------------------------------------------------------------//
void MOAIForceAction::Apply ( float step ) {
	...
	for ( u32 i = 0; i < total; ++i ) {
		...
		MOAIForceAttr& forceAttr = this->mForces [ i ];
...
//----------------------------------------------------------------//
bool MOAIForceAction::IsBusy () {

	return ( this->mTime < 1.0f );
}

//----------------------------------------------------------------//
MOAIForceAction::MOAIForceAction () :
	mTime ( 0.0f ),
	mDelay ( 0.0f ),
...
//----------------------------------------------------------------//
void MOAIForceAction::OnUpdate ( float step ) {
	
	this->Apply ( step / this->mDelay );
}

//----------------------------------------------------------------//
void MOAIForceAction::RegisterLuaClass ( USLuaState& state ) {

	MOAIAction::RegisterLuaClass ( state );
}

//----------------------------------------------------------------//
void MOAIForceAction::RegisterLuaFuncs ( USLuaState& state ) {
...
//----------------------------------------------------------------//
void MOAIForceAction::ReserveForces ( u32 total ) {

	this->mForces.Init ( total );
}

//----------------------------------------------------------------//
void MOAIForceAction::SetForce ( u32 idx, MOAINode* target, u32 attrID, float force, u32 mode ) {

	if ( idx < this->mForces.Size ()) {
		
		MOAIForceAttr& forceAttr = this->mForces [ idx ];
...
//----------------------------------------------------------------//
void MOAIForceAction::SetForce ( u32 mode ) {
...
//----------------------------------------------------------------//
STLString MOAIForceAction::ToString () {
