...
int MOAIProfilerReportBox::_getBackgroundColor ( lua_State* L ) {
	...
	USColorVec color = USColor::Set ( self->mBackgroundColor );
...
int MOAIProfilerReportBox::_getRowColor ( lua_State* L ) {
	...
	USColorVec color = USColor::Set ( self->mRowColor );
...
int MOAIProfilerReportBox::_getTextColor ( lua_State* L ) {
	...
	USColorVec color = USColor::Set ( self->mTextColor );
...
void MOAIProfilerReportBox::Draw ( int subPrimID ) {
	...
	// Get current state
	const USMatrix4x4& orgUvMatrix = gfxDevice.GetUVTransform ();
	...
	USColorVec memColorLow, memColorMed, memColorHigh, memColorCur;
...
//----------------------------------------------------------------//
u32 MOAIProfilerReportBox::GetPropBounds ( USBox& bounds ) {
...
MOAIProfilerReportBox::MOAIProfilerReportBox () :
	mFont ( 0 ),
	mFontSize ( 0 ),
	mBackgroundColor ( USColorVec ( 0, 0, 0, 0.25f ).PackRGBA () ),
	mRowColor ( USColorVec ( 0.2f, 0.2f, 0.2f, 0.5f ).PackRGBA () ),
	mTextColor ( USColorVec ( 1, 1, 1, 1 ).PackRGBA () ),
	mOrientation ( 0 ),
	mUserMemory ( 0 ),
	mMemoryXRange ( 0, 0 ),
	mLineHeight ( 0 ),
