...
//----------------------------------------------------------------//
/**	@brief <tt>() layout ( self )</tt>\n
\n
	Perform layout algorithm to layout hierarchy.
	@param self (in)
*/
int MOAIFrame::_layout ( lua_State* L ) {
	LUA_SETUP ( MOAIFrame, "U" )
...
//----------------------------------------------------------------//
/**	@brief <tt>( returns ) setFit ( self, xFit, yFit )</tt>\n
\n
	Sets the fitting mode for each access.
	Can be MOAIFrame.FIT_EXPAND, MOAIFrame.FIT_CONTENT or MOAIFrame.FIT_ABSOLUTE.
	@param self (in)
	@param xFit (in)
	@param yFit (in)
*/
int	MOAIFrame::_setFit ( lua_State* L ) {
	LUA_SETUP ( MOAIFrame, "U" )
...
//----------------------------------------------------------------//
/**	@brief <tt>() setIdealSize ( self, xSize, ySize )</tt>\n
\n
	...
	@param ySize (in)
*/
int	MOAIFrame::_setIdealSize ( lua_State* L ) {
	LUA_SETUP ( MOAIFrame, "U" )
...
//----------------------------------------------------------------//
/**	@brief <tt>() setLayout ( self, xFit, yFit )</tt>\n
\n
	Sets the layout mode for each access.
	Can be MOAIFrame.LAYOUT_ALIGN_MIN, MOAIFrame.LAYOUT_ALIGN_MAX or MOAIFrame.LAYOUT_ALIGN_CENTER,
	MOAIFrame.LAYOUT_JUSTIFY_MIN, MOAIFrame.LAYOUT_JUSTIFY_MAX or MOAIFrame.LAYOUT_JUSTIFY_CENTER.
	In this case, MIN, CENTER and MAX refer to the border along each axis. So on the x axis, MIN would
	be left and MAX would be right.
	@param self (in)
	@param y (in)
*/
int	MOAIFrame::_setLayout ( lua_State* L ) {
	LUA_SETUP ( MOAIFrame, "U" )
...
//----------------------------------------------------------------//
/**	@brief <tt>() setMargin ( self, left, top, right, bottom )</tt>\n
\n
	Sets the interior margin of the layout. This is the space left between
	the edge of the layout and the content.
	@param self (in)
	@param left (in)
	@param top (in)
	@param right (in)
	@param bottom (in)
*/
int MOAIFrame::_setMargin ( lua_State* L ) {
	LUA_SETUP ( MOAIFrame, "U" )
...
//----------------------------------------------------------------//
/**	@brief <tt>() setWeight ( self, xWeight, yWeight )</tt>\n
\n
	Sets the stretch weight for stretchable layours (i.e. FIT_EXPAND). This is the
	proportional stretch relative to all other stretchable layouts. Factors are
	evaluated relative to each other, so any number space can be used: 1, 2, 5 is
	equivalent to 0.5, 1, 2.5.
	@param self (in)
	@param xWeight (in)
	@param yWeight (in)
*/
int	MOAIFrame::_setWeight ( lua_State* L ) {
	LUA_SETUP ( MOAIFrame, "U" )
...
//----------------------------------------------------------------//
float MOAIFrame::GetInnerSize ( u32 axis ) {
...
//----------------------------------------------------------------//
MOAIFrame* MOAIFrame::GetParentWidget () {

	if ( this->mParent ) {
		return this->mParent->AsType <  MOAIFrame >();
	}
	return 0;
}

//----------------------------------------------------------------//
u32 MOAIFrame::GetPerpAxis ( u32 axis ) {

	return (( axis + 1 ) & 0x00000001 );
}

//----------------------------------------------------------------//
USRect MOAIFrame::GetScissorRect () {
...
//----------------------------------------------------------------//
float MOAIFrame::GetSize ( u32 axis ) {

	return this->mSize [ axis ];
}

//----------------------------------------------------------------//
void MOAIFrame::Layout () {
...
//----------------------------------------------------------------//
void MOAIFrame::Layout ( u32 axis ) {
...
//----------------------------------------------------------------//
void MOAIFrame::RegisterLuaClass ( USLuaState& state ) {
...
//----------------------------------------------------------------//
void MOAIFrame::RegisterLuaFuncs ( USLuaState& state ) {
...
//----------------------------------------------------------------//
void MOAIFrame::SetLocByAxis ( u32 axis, float loc ) {
...
//----------------------------------------------------------------//
void MOAIFrame::SetParent ( MOAITransformBase2D* parent ) {
	...
	if ( parent ) {
		MOAIFrame* parentWidget = parent->AsType < MOAIFrame >();
		if ( parentWidget ) {
