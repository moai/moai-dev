...
namespace DockPanelSuite
{
	...
	public partial class DockPanel : Panel
	{
        ...
        protected override void OnLayout(LayoutEventArgs levent)
		{
            ...
            else if (DocumentStyle == DocumentStyle.DockingMdi) InvalidateWindowRegion();
		...
		public void SetPaneIndex(DockPane pane, int index)
		{
			...
			if (oldIndex == -1)
				throw(new ArgumentException(DockPanelSuite.Properties.Resources.DockPanel_SetPaneIndex_InvalidPane));

			if (index < 0 || index > Panes.Count - 1)
				if (index != -1)
					throw(new ArgumentOutOfRangeException(DockPanelSuite.Properties.Resources.DockPanel_SetPaneIndex_InvalidIndex));
	    ...
	    internal Form ParentForm
		{
			...
			{	
				if (!IsParentFormValid())
					throw new InvalidOperationException(DockPanelSuite.Properties.Resources.DockPanel_ParentForm_Invalid);
        ...
        {
            Control parent;
            if (DocumentStyle == DocumentStyle.DockingMdi ||DocumentStyle == DocumentStyle.SystemMdi) parent = this.Parent;
            else parent = this;
