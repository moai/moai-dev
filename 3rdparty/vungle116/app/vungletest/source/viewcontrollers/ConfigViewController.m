...
-(VGConfigGroup*)groupControls
{
    ...
    [cgrp itemAdd:self.itemConfigBaseUrl];
...
-(VGConfigItem*)itemAdOrientation
{
    ...
    for (VGAdOrientation orient = VGAdOrientationFirst;
     orient <= VGAdOrientationLast; orient++)
    {
...
#if VG_DEBUG == 0
    isub = [VGConfigItem itemWithType:VGConfButtonChooserItem];
    isub.label = [VGUserData baseUrlString:VGBaseUrlFirst];
    [isub setValueInteger:VGBaseUrlFirst];
    [item.group itemAdd:isub];
    return item;
#endif
    for (VGConfigBaseUrl url = VGBaseUrlFirst;
         url <= VGBaseUrlLast; url++)
    {
        isub = [VGConfigItem itemWithType:VGConfButtonChooserItem];
