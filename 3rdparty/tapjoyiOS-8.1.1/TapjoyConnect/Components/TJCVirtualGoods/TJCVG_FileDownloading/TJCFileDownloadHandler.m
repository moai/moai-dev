...
- (BOOL)isItemPresentInLocalBD:(TJCVGStoreItem*)item
{
	...
	if ([[TJCSqliteController sharedInstance] storeItemIDExists:[item itemID]]) 
	{
		return YES;
	}
...
-(void) downloadItem:(TJCVGStoreItem *)obj
{
	...
	//    // Return if item already present in DB
	//	if ([[TJCSqliteController sharedInstance] storeItemIDExists:[obj itemID]]) 
	//	{
...
-(UIView*) getVGPurchasedItemDownloadProgressView:(NSString*) vGStoreItemId
{
	...
	{	
		if ([[TJCSqliteController sharedInstance] storeItemIDExists:vGStoreItemId]) 
		{
