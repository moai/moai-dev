...
#================================================================#
	...
	usage="usage: $0 [-v] [-i thumb | arm] [-a all | armeabi | armeabi-v7a] [-l appPlatform] [--use-untz true | false] [--disable-adcolony] [--disable-billing] [--disable-chartboost] [--disable-crittercism] [--disable-facebook] [--disable-push] [--disable-tapjoy]"
	verbose=
	arm_mode=arm
	...
	if [ -f libs/package.txt ]; then
		...
		existing_use_untz=$( sed -n '4p' libs/package.txt )
		existing_adcolony_flags=$( sed -n '5p' libs/package.txt )
		existing_billing_flags=$( sed -n '6p' libs/package.txt )
		existing_chartboost_flags=$( sed -n '7p' libs/package.txt )
		existing_crittercism_flags=$( sed -n '8p' libs/package.txt )
		existing_facebook_flags=$( sed -n '9p' libs/package.txt )
		existing_push_flags=$( sed -n '10p' libs/package.txt )
		existing_tapjoy_flags=$( sed -n '11p' libs/package.txt )
	...
	pushd jni > /dev/null
		...
		sed -i.backup s%@USE_FMOD@%"false"%g OptionalComponentsDefined.mk
