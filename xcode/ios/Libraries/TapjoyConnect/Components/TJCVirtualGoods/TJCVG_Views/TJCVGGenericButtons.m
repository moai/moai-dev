...
- (UILabel*)createButtonTitle:(NSString*)title WithFrame:(CGRect)frame
{
	
	UILabel *navBarBtnLabel = [[UILabel alloc] initWithFrame:CGRectMake(0,0,frame.size.width,frame.size.height)];
	
	[navBarBtnLabel setBackgroundColor:[UIColor clearColor]];
	[navBarBtnLabel setTextColor:[UIColor whiteColor]]; 
	[navBarBtnLabel setFont:[UIFont fontWithName:@"Arial-BoldMT" size:12.5f]];
	[navBarBtnLabel setText:title];
	[navBarBtnLabel setTextAlignment:UITextAlignmentCenter];
	
	return navBarBtnLabel;
}
...
- (void)setStandardTintColorWithTitle:(NSString*)title withParentView:(UIView*)view
{
	...
	[segmentButton setTintColor: [[TJCViewCommons sharedObject] getUserDefinedColor]];
	[view addSubview:segmentButton];
	customColor = [[TJCViewCommons sharedObject] getUserDefinedColor];
...
- (void)buttonActionTouchDown:(id)sender
{
	...
	int oldColor = [[TJCViewCommons sharedObject] getUserDefinedColorCode];
